...
# === НАСТРОЙКИ ОТВЕТОВ =================================================
SELECTED_SWARM="A"         # Варианты: "A" — Math, "B" — Math Hard
PARAM_B_CHOICE="0.5"       # Варианты: 0.5, 1.5, 7, 32, 72
HF_PUSH_MODELS="N"         # Варианты: "Y" — да, "N" — нет
HF_TOKEN_INPUT=""          # Укажите токен, если HF_PUSH_MODELS="Y"
# =======================================================================
...
if [ -n "${HF_TOKEN}" ]; then
    ...
    case "$yn" in
        [Yy]* ) 
            HUGGINGFACE_ACCESS_TOKEN=HF_TOKEN_INPUT
            ;;
        [Nn]* ) 
